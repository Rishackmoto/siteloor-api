...
router.delete('/api/pengajuan/:id', async (req, res) => {
    try {
        ...
        // lanjut hapus file di background
        setImmediate(async () => {
            try {
                console.log(
                    `[BACKGROUND] cleanup storage ${id}`
                );

                const deletedFiles =
                    await cleanupPengajuanStorage(pool, id);

                console.log(
                    `[BACKGROUND] cleanup selesai ${id}, file=${deletedFiles}`
                );
            } catch (err) {
                console.error(
                    `[BACKGROUND] cleanup gagal ${id}:`,
                    err.message
                );
            }
        });
...
router.initializeDatabase = async function initializeDatabase() {
  const pool = await getPool();
