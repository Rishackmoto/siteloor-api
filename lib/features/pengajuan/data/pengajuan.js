...
router.get('/api/dashboard/summary', async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      WITH jenis AS (
        SELECT 'telur_besar' AS key_name, '11' AS jns, 'besar' AS kolom
        UNION ALL SELECT 'telur_sedang', '12', 'sedang'
        UNION ALL SELECT 'telur_kecil', '13', 'kecil'
        UNION ALL SELECT 'telur_retak', '14', 'retak'
        UNION ALL SELECT 'telur_kecil_sekali', '15', 'ssekali'
        UNION ALL SELECT 'telur_sangat_kecil', '16', 'sekali'
      ),
      produksi AS (
        SELECT
          SUM(ISNULL(besar,0)) AS besar,
          SUM(ISNULL(sedang,0)) AS sedang,
          SUM(ISNULL(kecil,0)) AS kecil,
          SUM(ISNULL(retak,0)) AS retak,
          SUM(ISNULL(ssekali,0)) AS ssekali,
          SUM(ISNULL(sekali,0)) AS sekali
        FROM transaksi_telur
      ),
      keluar AS (
        SELECT
          jns,
          SUM(ISNULL(jmlh,0)) * 30 AS total_keluar
        FROM transaksi_jual
        WHERE flag = 'JTL'
        GROUP BY jns
      )
      SELECT
        ISNULL(p.besar,0) - ISNULL((SELECT total_keluar FROM keluar WHERE jns='11'),0) AS telur_besar,
        ISNULL(p.sedang,0) - ISNULL((SELECT total_keluar FROM keluar WHERE jns='12'),0) AS telur_sedang,
        ISNULL(p.kecil,0) - ISNULL((SELECT total_keluar FROM keluar WHERE jns='13'),0) AS telur_kecil,
        ISNULL(p.retak,0) - ISNULL((SELECT total_keluar FROM keluar WHERE jns='14'),0) AS telur_retak,
        ISNULL(p.ssekali,0) - ISNULL((SELECT total_keluar FROM keluar WHERE jns='15'),0) AS telur_kecil_sekali,
        ISNULL(p.sekali,0) - ISNULL((SELECT total_keluar FROM keluar WHERE jns='16'),0) AS telur_sangat_kecil
      FROM produksi p;
    `);

    res.json({
      success: true,
      title: 'SUMMARY STOK TELUR AKHIR',
