...
router.get('/api/pengambilan-telur/noref', async (req, res) => {
  try {
    ...
    const prefixUser = (userid || 'USER').substring(0, 4).toUpperCase();

    const pool = await getPool();

    const result = await pool.request()
      .input('prefix', sql.VarChar, `${prefixUser}TL%`)
      .query(`
    ...
    const noref = `${prefixUser}TL${nomor.toString().padStart(6, '0')}`;

    res.set('Cache-Control', 'no-store');
    res.json({ success: true, noref });
  } catch (err) {
    console.error('Gagal generate noref telur:', err);
    res.status(500).json({
      success: false,
