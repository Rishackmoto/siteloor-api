...
router.get('/api/pengambilan-telur/noref', async (req, res) => {
  try {
    ...
    const luserid = userid.toString().substring(0, 4).toUpperCase();

    const pool = await getPool();

    const result = await pool.request().query(`
      SELECT MAX(RIGHT(noref, 6)) AS nomor
      FROM transaksi_telur
      WHERE noref LIKE '%TL%'
    `);

    let nomor = result.recordset[0]?.nomor;

    if (!nomor) {
      nomor = 1;
    } else {
      nomor = parseInt(nomor, 10) + 1;
    }

    if (nomor > 999999) {
      return res.status(400).json({
        message: 'No. referensi sudah maksimal',
      });
    }

    const nomorUrut = nomor.toString().padStart(6, '0');
    const noref = `${luserid}TL${nomorUrut}`;
    ...
    console.error('GENERATE NOREF PENGAMBILAN TELUR ERROR:', err);
    res.status(500).json({
      message: 'Gagal generate noref',
    });
  }
});
...
router.get('/api/pengambilan-ayam/noref', async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT MAX(TRY_CONVERT(INT, RIGHT(noref, 6))) AS nomor
