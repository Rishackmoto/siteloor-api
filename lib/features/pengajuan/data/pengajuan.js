...
router.get('/api/dashboard/summary', async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      DECLARE @tgl_terakhir date;

      SELECT @tgl_terakhir = MAX(tgl)
      FROM transaksi_telur;

      SELECT
        @tgl_terakhir AS tgl,
        ISNULL(SUM(besar),0) AS telur_besar,
        ISNULL(SUM(sedang),0) AS telur_sedang,
        ISNULL(SUM(kecil),0) AS telur_kecil,
        ISNULL(SUM(retak),0) AS telur_retak,
        ISNULL(SUM(ssekali),0) AS telur_kecil_sekali,
        ISNULL(SUM(sekali),0) AS telur_sangat_kecil
      FROM transaksi_telur
      WHERE tgl = @tgl_terakhir
    `);
