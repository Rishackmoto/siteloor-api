  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        -- 🔥 CTE untuk total keseluruhan dalam rentang
        total_masuk AS (
        ...
        SELECT
          ...
          -- Saldo dalam butir (sudah memperhitungkan penjualan sebelum tgl1)
          (
            SELECT SUM(ISNULL(tt.total, 0))
            FROM telur_total tt
            WHERE tt.tgl <= k.tgl
          ...
          -- 🔥 Tambahkan total agregat sebagai kolom (akan sama di setiap baris)
    ...
    // Ambil total dari baris pertama (semua baris memiliki nilai yang sama)
