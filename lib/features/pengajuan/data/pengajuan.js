...
router.get('/api/dashboard/stok-telur', async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        SELECT
          ...
          -- Saldo dalam butir = total penerimaan sampai tgl ini - (opening_outgoing + kumulatif penjualan dari tgl1)
          (
            SELECT SUM(ISNULL(tt.total, 0))
            FROM telur_total tt
            WHERE tt.tgl <= k.tgl
    ...
    // Hitung total agregat untuk rentang
    let totalMasukBtr = 0, totalKeluarBtr = 0;
    let saldoAkhirBtr = 0, saldoAkhirRak = 0;
    if (records.length > 0) {
      records.forEach(r => {
        totalMasukBtr += r.btr_masuk;
        totalKeluarBtr += r.btr_keluar;
      });
      const last = records[records.length - 1];
      saldoAkhirBtr = last.btr_saldo;
      saldoAkhirRak = last.rak_saldo;
    }
    ...
    res.json({
      ...
      total: {
        masuk: {
          butir: totalMasukBtr,
          rak: parseFloat((totalMasukBtr / 30).toFixed(2))
        },
        keluar: {
          butir: totalKeluarBtr,
          rak: parseFloat((totalKeluarBtr / 30).toFixed(2))
        },
        saldo: {
          butir: saldoAkhirBtr,
          rak: saldoAkhirRak
