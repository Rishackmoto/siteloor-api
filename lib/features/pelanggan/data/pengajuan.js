...
router.put('/api/users/:userid/password', async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(String(pass).trim(), 10);
...
router.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT TOP 1
          ...
          foto_profile,
          ${selectColumn(muserColumns, 'theme_mode', "'light'")}
        FROM muser
        WHERE userid = @userid
          ${activeUserFilter}
      `);
    ...
    const passwordInput = String(pass).trim();
    const passwordDb = String(user.pass || '').trim();

    let validPassword = false;
    let needMigrateToBcrypt = false;

    if (
      passwordDb.startsWith('$2a$') ||
      passwordDb.startsWith('$2b$') ||
      passwordDb.startsWith('$2y$')
    ) {
      validPassword = await bcrypt.compare(passwordInput, passwordDb);
    } else {
      validPassword = passwordDb === passwordInput;
      needMigrateToBcrypt = validPassword;
    }
    ...
    if (needMigrateToBcrypt) {
      const newHash = await bcrypt.hash(passwordInput, 10);

      await pool.request()
        .input('userid', sql.VarChar(30), String(userid).trim())
        .input('pass', sql.VarChar(255), newHash)
        .query(`
          UPDATE muser
          SET pass = @pass
          WHERE userid = @userid
...
router.get('/api/dashboard/produksi-penjualan', async (req, res) => {
  try {
    ...
    const jenisKey = String(req.query.jenis || 'kecil').trim().toLowerCase();
    const config = jenisMap[jenisKey];
      ...
      .query(`
        ...
        produksi AS (
          SELECT
            ...
            SUM(ISNULL(${config.kolom}, 0)) AS pengambilan
          FROM transaksi_telur
        ...
        SELECT
          CONVERT(char(7), b.awal_bulan, 120) AS periode,
          FORMAT(b.awal_bulan, 'MMM yyyy', 'id-ID') AS bulan,
          b.awal_bulan,
          ISNULL(p.pengambilan, 0) AS pengambilan,
          ISNULL(p.pengambilan, 0) AS produksi,
...
router.put('/api/users/:userid/theme-mode', async (req, res) => {
  try {
    ...
    const mode = String(theme_mode || '').trim().toLowerCase();
    const allowedModes = new Set(['light', 'dark', 'clay', 'bento']);

    if (!allowedModes.has(mode)) {
      return res.status(400).json({
        success: false,
        message: 'Theme mode tidak valid',
      });
    }
    ...
    return res.status(500).json({
      success: false,
      message: 'Gagal menyimpan mode tema',
      error: error.message,
    });
  }
});
