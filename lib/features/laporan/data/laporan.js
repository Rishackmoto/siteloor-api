...
router.get('/summary-piutang-sales', async (req, res) => {
  try {
    const { sales, tgl1, tgl2 } = req.query;
    ...
    const request = pool.request()
      .input('cari', sql.VarChar, cari)
      .input('tgl1', sql.Date, tgl1 || null)
      .input('tgl2', sql.Date, tgl2 || null);
    ...
    const result = await request.query(`
      ;WITH piutang AS (
        SELECT
          ...
          SUM(ISNULL(tj.total,0)) AS piutang,
          ISNULL((
            SELECT SUM(ISNULL(tp.nominal,0))
            FROM transaksi_piutang tp
            WHERE tp.noref2 = tj.noref
          ),0) AS bayar
        FROM transaksi_jual tj
        INNER JOIN jenissales js
          ON tj.sales = js.kode
        WHERE js.jenis LIKE @cari
          AND tj.stsbyr = 'HT'
          AND (
                (@tgl1 IS NULL AND @tgl2 IS NULL)
             OR (@tgl1 IS NOT NULL AND @tgl2 IS NULL AND tj.tgl >= @tgl1)
             OR (@tgl1 IS NULL AND @tgl2 IS NOT NULL AND tj.tgl <= @tgl2)
             OR (@tgl1 IS NOT NULL AND @tgl2 IS NOT NULL
                 AND tj.tgl BETWEEN @tgl1 AND @tgl2)
          )
        GROUP BY
          tj.noref,
          tj.pelanggan,
    ...
    console.error('GET SUMMARY PIUTANG SALES ERROR:', err);

    res.status(500).json({
      success: false,
      message: 'Gagal mengambil summary piutang sales',
      error: err.message,
    });
  }
});
router.get('/pemakaian-pakan', async (req, res) => {
  try {
    const { tgl1, tgl2 } = req.query;
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT
          ISNULL(noref, '') AS noref,
          ISNULL(cnm, '') AS kandang,
          tgl,
          ISNULL(ket, '') AS ket,
          ISNULL(jns, '') AS jenis,
          ISNULL(nm, '') AS ket_pakan,
          ISNULL(qty2, 0) AS jmlh,
          ISNULL(satuan, '') AS satuan
        FROM transaksi_kelola
        WHERE tgl >= @tgl1
          AND tgl < DATEADD(day, 1, @tgl2)
          AND LEFT(noref, 2) = 'TP'
        ORDER BY tgl ASC, noref ASC
    ...
    console.error('LAPORAN PEMAKAIAN PAKAN ERROR:', err);
    res.status(500).json({
      success: false,
      message: 'Gagal mengambil laporan pemakaian pakan',
      error: err.message,
    });
  }
});

router.get('/stok-pakan', async (req, res) => {
  try {
    const { tgl1, tgl2 } = req.query;
    ...
    const rows = result.recordset.map((r, i) => {
      const stok = Number(r.qty || 0);
      const sisa = Number(r.qty2 || 0);
    ...
    console.error('LAPORAN STOK PAKAN ERROR:', err);
...
router.get('/pemakaian-obat', async (req, res) => {
  try {
    const { tgl1, tgl2 } = req.query;
    ...
    console.error('LAPORAN PEMAKAIAN OBAT ERROR:', err);
    res.status(500).json({
      success: false,
      message: 'Gagal mengambil laporan pemakaian obat',
      error: err.message,
    });
  }
});
